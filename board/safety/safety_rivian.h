...
static void rivian_rx_hook(const CANPacket_t *to_push) {
  ...
  if (bus == 2) {
    if (rivian_longitudinal && (addr == 0x2b9)) {
      // "AEB_ACTIVE"
      rivian_stock_aeb = ((GET_BYTE(to_push, 2) & 0x03U) == 1U);
    }
  }
...
static bool rivian_tx_hook(const CANPacket_t *to_send) {
  ...
  if(addr == 0x2b9) {
    ...
    if (rivian_longitudinal) {
      ...
      violation |= longitudinal_accel_checks(raw_accel_max, RIVIAN_LONG_LIMITS);
      violation |= longitudinal_accel_checks(raw_accel_min, RIVIAN_LONG_LIMITS);
    } else {
      violation = true;
    }
  }
...
static int rivian_fwd_hook(int bus_num, int addr) {
  ...
  if(bus_num == 2) {
    ...
    if (rivian_longitudinal && (addr == 0x2b9) && !rivian_stock_aeb) {
      block_msg = true;
    }
